...
from sklearn.metrics import roc_auc_score, precision_recall_curve, auc, f1_score,r2_score, precision_score, recall_score
import random
...
def calculate_precision_recall(y_true, y_pred):
    ...
    if torch.unique(y_pred).shape[0] == 1:
        ...
        return 0,0,0
    precision, recall, threshold = precision_recall_curve(y_true.squeeze().detach().cpu().numpy(), y_pred.squeeze().detach().cpu().numpy())
    auprc = auc(recall, precision)
    precision = precision_score(y_true.detach().cpu().numpy(), (y_pred > 0.5).float().detach().cpu().numpy())
    recall = recall_score(y_true.detach().cpu().numpy(), (y_pred > 0.5).float().detach().cpu().numpy())
...
def evaluate_merged(model, dataloader, device,task_list=[1,2,3,4,5,6,7,8,9]):
    ...
    with torch.no_grad():
        for batch_idx, (tuple_, task) in enumerate(dataloader):
            ...
            if task[0] ==5:
                ...
                correct_predictions_5 += (masked_predicted == masked_targets).sum().item()
            ...
            if task[0] ==8:
                ...
                correct_predictions_8 += (masked_predicted == masked_targets).sum().item()
    ...
    if 2 in task_list:
        ...
        precision_task2, recall_task2, auprc_task2 = calculate_precision_recall(all_targets_task2, all_preds_task2)
    ...
    if 3 in task_list:
        ...
        precision_task3, recall_task3, auprc_task3 = calculate_precision_recall(all_targets_task3, all_preds_task3)
    ...
    if 4 in task_list:
        ...
        precision_task4, recall_task4, auprc_task4 = calculate_precision_recall(all_targets_task4, all_preds_task4)
    ...
    if 6 in task_list:
        ...
        precision_task6, recall_task6, auprc_task6 = calculate_precision_recall(all_targets_task6, all_preds_task6)
    ...
    if 7 in task_list:
        ...
        precision_task7, recall_task7, auprc_task7 = calculate_precision_recall(all_targets_task7, all_preds_task7)
    ...
    if 9 in task_list:
        ...
        precision_task9, recall_task9, auprc_task9 = calculate_precision_recall(all_targets_task9, all_preds_task9)
    ...
    return (accuracy_1,
            [acc_task2, auroc_task2, precision_task2 , recall_task2, auprc_task2, f1_score_task2],
            [acc_task3, auroc_task3, precision_task3 , recall_task3, auprc_task3, f1_score_task3],
            [acc_task4, auroc_task4, precision_task4 , recall_task4, auprc_task4, f1_score_task4],
            accuracy_5,
            [acc_task6, auroc_task6, precision_task6 , recall_task6, auprc_task6, f1_score_task6],
            [acc_task7, auroc_task7, precision_task7 , recall_task7, auprc_task7, f1_score_task7],
            accuracy_8,
            [acc_task9, auroc_task9, precision_task9 , recall_task9, auprc_task9, f1_score_task9])
