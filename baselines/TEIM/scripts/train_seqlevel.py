...
class SeqLevelSystem(pl.LightningModule):
    ...
    def minimum_step(self, batch, device=None):
        ...
        if device is None:
            CDR3B, Epitope, Affinity = batch['CDR3B'], batch['Epitope'], batch['Affinity']
        else:
            CDR3B, Epitope, Affinity = batch['CDR3B'].to(device), batch['Epitope'].to(device), batch['Affinity'].to(device)
        pred = self([CDR3B, Epitope])
        loss = self.get_loss(pred, Affinity)
        return loss, Affinity, pred

    def training_step(self, batch, batch_idx):
        self.train()
        loss, Affinity, pred = self.minimum_step(batch)
    ...
    def get_loss(self, pred, Affinity):
        loss = F.binary_cross_entropy(pred.view(-1), Affinity.float(), weight=None, reduction='mean')
        return loss
...
if __name__ == '__main__':
    ...
    for i_split, (train_set_this, val_set_this) in enumerate(zip(train_set, val_set)):
        ...
        if len(results) == 4:
            pd.DataFrame(zip(*results), columns=['CDR3B', 'Epitope', 'y_true', 'y_pred']).to_csv(os.path.join(trainer.log_dir, 'val_pred.csv'), index=False)
        else:
            pd.DataFrame(zip(*results), columns=['CDR3B', 'Epitope', 'y_true', 'y_pred', 'epi_id']).to_csv(os.path.join(trainer.log_dir, 'val_pred.csv'), index=False)
        print('Done')